...
public class CameraMovement : MonoBehaviour {
    ...
    [SerializeField] private float sprintSpeedAdd = 0.1f;
    ...
    private void HandleMovement() {
        ...
        //moveDir = transform.TransformDirection(moveDir) * currentSpeed * Time.unscaledDeltaTime;
        //rb.MovePosition(rb.position + moveDir);

        moveDir = transform.TransformDirection(moveDir) * Time.unscaledDeltaTime;
        ...
        //bool canMoveVertically = !Physics.BoxCast(boxCastCenter, boxSize / 2, Vector3.up, Quaternion.identity, moveDistance, collisionsLayerMask);

        Vector3 verticalMove = Vector3.up * (upInput + downInput) * currentSpeed;
        transform.position += verticalMove * Time.unscaledDeltaTime;
