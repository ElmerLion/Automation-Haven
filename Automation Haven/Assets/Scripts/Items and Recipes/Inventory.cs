...
public class Inventory {
    ...
    public Inventory(int maxInventorySlots, InventoryType type = InventoryType.Storage, int slotsPerItem = 0) {
    ...
    public bool TryAddItem(ItemAmount itemAmount, List<ItemObject> itemObjects = null) {
        if (itemObjects == null) itemObjects = new List<ItemObject>();
        ...
        foreach (InventorySlot inventorySlot in inventorySlotsCopy) {
            if (inventorySlot != null) {
                ...
                if (itemObject != null && inventorySlot.itemObjectList.Contains(itemObject)) {
                    ...
                    OnItemRemoved?.Invoke(itemObject);
                }
                ...
                if (itemObject == null) {
                    for (int i = 0; i < amount; i++) {
                        ...
                        OnItemRemoved?.Invoke(inventorySlot.itemObjectList[0]);
                    }
                }
