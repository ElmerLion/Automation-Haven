...
public class RecipeManager : MonoBehaviour {
    ...
    private void Start() {
        ...
        // Loads in all recipes to their respective levels
        /*foreach (RecipeSO recipeSO in allRecipes) {
            if (!recipesInEachLevelDic.ContainsKey(recipeSO.output[0].itemSO.unlockedAtLevel)) {
                recipesInEachLevelDic.Add(recipeSO.output[0].itemSO.unlockedAtLevel, new List<RecipeSO>());
            }
            recipesInEachLevelDic[recipeSO.output[0].itemSO.unlockedAtLevel].Add(recipeSO);
        }

        foreach (RecipeSO recipeSO in recipesInEachLevelDic[1]) {
            unlockedRecipes.Add(recipeSO);
        }*/
