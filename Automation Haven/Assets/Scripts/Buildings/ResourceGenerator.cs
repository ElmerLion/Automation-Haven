...
public class ResourceGenerator : MonoBehaviour, IHasProgress {
    ...
    public void CheckResourceNearbyStatus() {
        Debug.Log("Checking resource nodes");

        if (itemSOResourceNodeAmountDic == null) {
        ...
        if (itemSOResourceNodeAmountDic.Count <= 0) {
            Debug.Log("Found no resource nodes");
